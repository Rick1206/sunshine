package code.page.brand
{
	...
	public class nutMain extends MovieClip
	{
		...
		private var curRmc:mcPro;
		private var curLmc:mcPro;
		...
		private function onCompInfoHandle(e:Event):void
		{
			...
			for (var i:int = 0; i < 9; i++) {
				...
				if (i % 2 == 0 && i < 8)
				{
					var myNut:mpro = new mpro(xmlData.pro[i].type, xmlData.pro[i].title, xmlData.pro[i].description, xmlData.pro[i].link, xmlData.pro[i].img);
			...
			for (i = 0; i < 9; i++)
			{
				...
				if (i % 2 != 0 && i > 0)
				{
					...
					myNut = new mpro(xmlData.pro[i].type, xmlData.pro[i].title, xmlData.pro[i].description, xmlData.pro[i].link, xmlData.pro[i].img);
